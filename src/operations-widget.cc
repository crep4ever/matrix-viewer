...
COperationWidget::COperationWidget(const QString& p_title, CMatrixModel* p_model, QWidget* p_parent)
    : QWidget(p_parent)
...
{
    model()->convertTo(m_typeWidget->currentIndex() + 8 * (model()->channels() - 1), m_alphaWidget->value(), m_betaWidget->value());
}
...
void CRotationWidget::apply()
{
    ...
    model()->rotate(m_centerWidget->point(), m_angleWidget->value(), m_scaleWidget->value());
}
...
void CNormalizeWidget::apply()
{
    ...
    model()->normalize(m_alphaWidget->value(), m_betaWidget->value(), norm);
}
    ...
    , m_mulTransposeWidget(new QPushButton(tr("Multiply by transposed"), this))
{
    ...
    connect(m_transposeWidget, SIGNAL(clicked()), model(), SLOT(transpose()));

    connect(m_verticalFlipWidget, SIGNAL(clicked()), model(), SLOT(verticalFlip()));

    connect(m_horizontalFlipWidget, SIGNAL(clicked()), model(), SLOT(horizontalFlip()));

    connect(m_mulTransposeWidget, SIGNAL(clicked()), model(), SLOT(mulTranspose()));
}
...
void CThresholdWidget::apply()
{
    ...
    model()->threshold(m_thresholdValueWidget->value(), m_maxValueWidget->value(), type);
}
    ...
    , m_multiplyMatrixWidget(new QPushButton(tr("Matrix multiply"), this))
{
    ...
    connect(m_absDiffWidget, SIGNAL(clicked()), this, SLOT(absDiff()));

    connect(m_multiplyElementsWidget, SIGNAL(clicked()), this, SLOT(multiplyElements()));

    connect(m_multiplyMatrixWidget, SIGNAL(clicked()), this, SLOT(multiplyMatrix()));
}
...
void CChannelsWidget::apply()
{
    ...
    if (m_redOpenPath.isEmpty() || m_greenOpenPath.isEmpty() || m_blueOpenPath.isEmpty())
    {
