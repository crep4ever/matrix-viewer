...
const QStringList CMainWindow::_fileExtensions = QStringList() << "*.mfe"
                                                               << "*.MFE"
                                                               << "*.raw"
                                                               << "*.RAW"
                                                               << "*.png"
...
void CMainWindow::newMatrix()
{
    ...
    QString filename = QString("matrix_%1x%2_%3.mfe").arg(QString::number(rows)).arg(QString::number(cols)).arg(model->typeString(true));
...
void CMainWindow::loadProfile()
{
    ...
    QString filename =
        QFileDialog::getOpenFileName(nullptr, tr("Open profile file"), QString(PROJECT_DATA_PATH) + "/profiles", tr("Profile files (*.xml)"));
...
void CMainWindow::saveAs()
{
    ...
    QString filename = QFileDialog::getSaveFileName(nullptr, tr("Save data file"), m_savePath, tr("Matrices (%1)").arg(_fileTypeFilters.join(" ")));
...
void CMainWindow::changeTab(int p_index)
{
    ...
    if (currentWidget() != nullptr)
    {
        ...
        connect(positionWidget(), SIGNAL(positionChanged(int, int)), currentWidget(), SLOT(selectItem(int, int)));
